...
// close current scope
   public void CloseScope() {
      ...
      while(obj !=null){
        ...
        Console.WriteLine(";name: " + obj.name + ", kind: " +  kind + ", type: "+ type + ", level:" + (obj.level==0?" global":" local") + ", adr:" + obj.adr);
        obj = obj.next;
      }
...
// create new object node in current scope
   public Obj NewObj(string name, int kind, int type, int size=1, int size2=1) {
      Obj p, last; 
      Obj obj = new Obj();
      obj.ConstHas = false;
      obj.length = size;
      ...
      if (kind == array){
         obj.adr = topScope.nextAdr++;
         //the array is stored, the first element is the next local variable, the nth element is the nth local variable after the array object
         topScope.nextAdr += (size*size2);    
      }
      return obj;
   }
